...
function MyOrders() {
  ...
  const handleCancelOrder = async (order) => {
    ...
    if (!orderId) {
      window.alert("Unable to cancel this order because the order ID is missing.");
      return;
    }
    ...
    try {
      ...
      setOrders((currentOrders) => currentOrders.map((currentOrder) => {
        const currentId = getOrderId(currentOrder);

        if (currentId !== orderId) return currentOrder;
    ...
    <section className="my-orders-page">
      ...
      <div className="orders-list">
        {orders.map((order, index) => {
          const orderId = getOrderId(order) || `order-${index}`;
          const orderNumber = getOrderNumber(order);
              ...
              <div className="order-actions">
                ...
                {canCancelOrder(order) && (
                  ...
                  >
                    {isCancelling ? (
                      <>
                        <FiLoader className="button-spinner" aria-hidden="true" />
                        Cancelling...
                      </>
                    ) : (
                      <>
